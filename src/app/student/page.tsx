...
function StudentContent() {
  ...
  const handleAuthorizeCompliance = async (reqId: string) => {
    setComplianceRequests((prev) =>
      prev.map((r) =>
        r.id === reqId
          ? { ...r, status: "SHARED", shared_at: new Date().toISOString() }
          : r
      )
    );
    try {
                  ...
                  <div className="pt-4 mt-4 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-2 text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-navy-800 hover:bg-slate-700 text-slate-300 hover:text-solana-green transition-all"
                      >
            ...
            <form onSubmit={handleSubmitRequest} className="space-y-4 text-xs">
              ...
              <div className="flex items-center gap-2 pb-3 border-b border-slate-800/80">
                ...
                <button
                  type="button"
                  onClick={() => {
                    setReqTitle("Workshop de Rust e Smart Contracts na Solana");
                    setReqHours("40");
                    setReqOrigin("EXTERNAL");
                    setReqExternalIssuer("Superteam Academy");
                    setReqNotes("Curso intensivo on-chain com desenvolvimento prático de Solana Programs.");
                  }}
                  className="rounded-lg bg-solana-green/10 hover:bg-solana-green/20 text-solana-green px-2.5 py-1 border border-solana-green/30 transition"
                >
                  ⚡ Workshop de Rust na Solana (40h)
                </button>
              </div>
